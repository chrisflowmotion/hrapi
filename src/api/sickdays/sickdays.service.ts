...
import { ISickDay, IUpdateSickDay } from "./sickdays.model";
...
export const getEmployeeSickDays = async (employeeID: IEmployee['id']) => {
    return await execute<ISickDay[]>(SickDayQueries.getEmployeeSickDays, [employeeID]);
};
...
export const deleteSickDay = async (sickDayID: ISickDay['id']) => {
    const result = await execute<{ affectedRows: number }>(SickDayQueries.deleteSickDay, [sickDayID]);
    return result.affectedRows > 0;
};
